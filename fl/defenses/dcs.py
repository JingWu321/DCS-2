...
def defense_optim(args, model, loss_fn, gt_imgs, gt_labels, dm, ds, device, proxy_imgs, proxy_labels, save_path):
    ...
    for j in range(args.dcs_iter):
        closure = defense_closure(args, model.to(device), optimizer, my_criterion,
                                  my_sen_img.to(device), my_senout.to(device), sen_g, proxy_imgs.to(device), proxy_labels.to(device))
    ...
    adv_img = torch.cat([proxy_imgs.to(device), gt_imgs.to(device)], dim=0).to(device)
    adv_label = torch.cat([proxy_labels.to(device), gt_labels.to(device)], dim=0).to(device)
    if args.demo:
    ...
    loss = ((args.lambda_y * loss_fn(adv_out[:args.num_sen * args.per_adv], proxy_labels) \
            + (1 - args.lambda_y) * loss_fn(adv_out[:args.num_sen * args.per_adv], sen_label)) \
