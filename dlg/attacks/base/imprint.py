...
class ImprintBlock(torch.nn.Module):
    ...
    def __init__(self, data_size, num_bins, connection="linear", gain=1e-3, linfunc="fourier", mode=0):
        ...
        self.bins = self._get_bins(linfunc=linfunc)
        with torch.no_grad():
    ...
    def _init_linear_function(self, linfunc="fourier", mode=0):
        ...
        if linfunc == "avg":
            ...
            weights = torch.cos(math.pi / N * (torch.arange(0, N) + 0.5) * mode).repeat(K, 1) / N * max(mode, 0.33) * 4  # 0.1320, KxN
    ...
    def _get_bins(self, linfunc="avg"):
        ...
        for i in range(1, self.num_bins): # from 1 to num_bins-1
